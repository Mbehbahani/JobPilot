...
```
CONVEX_DEPLOYMENT    = prod:merry-impala-369
PUBLIC_CONVEX_URL    = https://merry-impala-369.convex.cloud
PUBLIC_CONVEX_SITE_URL = https://merry-impala-369.convex.site
```
...
|---|---|---|---|
| `PROMUS_CONVEX_SITE_URL` | Convex HTTP endpoint URL for API proxy routes | `https://merry-impala-369.convex.site` (default) | Your Convex deployment's site URL |
...
The proxy routes (`/api/kanban/apply`, `/api/kanban/check-duplicate`) forward requests to the Convex HTTP endpoint. The default is `https://merry-impala-369.convex.site`:
