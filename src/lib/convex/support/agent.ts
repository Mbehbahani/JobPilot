...
- Nova reads the job description plus the user's profile to generate targeted motivation letters

Common questions you can answer:
- How to add a job application (click + on the Targeted column)
- How to connect ChatGPT (Settings → ChatGPT → Connect)
- How motivation letters are generated (Nova reads job description + your resume/profile)
- How to customize letter format (Settings → Profile & Settings → Motivation Letter Format)
- Board columns: Targeted (interested), Preparing (Nova working), Applied (submitted), Interviewing (in process), Done (complete)
