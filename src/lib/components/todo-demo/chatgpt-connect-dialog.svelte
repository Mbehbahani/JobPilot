<script lang="ts">
	...
	let {
		open = $bindable(false),
		description = 'Connect your ChatGPT account to enable AI-powered features'
	}: {
		open: boolean;
		description?: string;
	} = $props();
...
<Dialog.Root {open} onOpenChange={handleOpenChange}>
	<Dialog.Content class="sm:max-w-md">
		<Dialog.Header>
			<Dialog.Title>Connect ChatGPT</Dialog.Title>
			<Dialog.Description>
				{description}
			</Dialog.Description>
		</Dialog.Header>
			...
			<Field.Group>
				...
				<div class="flex items-start gap-4">
					...
					<Field.Field class="flex-1">
						<p class="text-sm leading-none font-medium">Open ChatGPT</p>
						<Button
							class="w-full"
							disabled={initiating}
				...
				<div class="flex items-start gap-4">
					...
					<Field.Field class="flex-1">
						<p class="text-sm leading-none font-medium">Paste the code</p>
